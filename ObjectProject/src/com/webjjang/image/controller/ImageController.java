	...
	public void execute() {
		...
		while (true) {
			try {
				...
				// 메뉴 출력
				Output.menu("1.이미지 게시판리스트  2.이미지 게시판글보기  3.이미지 게시판글쓰기",
						"4.이미지 게시판글수정  5.이미지 게시판글삭제  0.이전메뉴",
						"글보기, 글수정, 글삭제를 하시려면 '1.이미지 게시판리스트'에서 먼저 글번호 확인요망");
				// 메뉴 입력
				...
				switch (menu) {
					case "1":
						...
						// 1. DB에서 데이터 가져오기
						List<ImageVO> list =
								(List<ImageVO>) ExeService.execute(new ImageListService(), null);
					...
					case "3":
						...
						int writeResult =
								(Integer) ExeService.execute(new ImageWriteService(), invo);
						// 3. 결과에 대한 출력
					...
					case "4":
						...
						ImageVO updateVO = (ImageVO) ExeService.execute(new ImageViewService(),
								Input.getLong("수정할 글번호 입력"));
						// 3. 화면에 뿌린다. - 글보기 출력 -> 수정처리 (updateImageVO)) while 안에서 출력 (4번과 합침)

						// 4. 수정처리를 한다. -> 메서드 작성
						updateImageVO(updateVO);
						// 위에 메서드 안에서 vo객체를 변경을 하면 밖에서도 변경이 된다.
						System.out.println("ImageController.execute() 수정완료 후 vo : " + updateVO);

						// 5. 수정한 내용을 DB에 저장 -> ImageUpdateService
						int updateResult =
								(Integer) ExeService.execute(new ImageUpdateService(), updateVO);
						// 6. 수정결과를 출력한다. print(result, msg)
						ImagePrint.print(updateResult, "이미지 게시판 글수정");
						break;
					case "5":
						// 삭제할 글번호를 받는다.
						// DB에 삭제처리 - ImageDeleteService
						int deleteResult = (Integer) ExeService.execute(new ImageDeleteService(),
