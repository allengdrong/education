...
public class ImageDAO {
	...
	public List<ImageVO> list() throws Exception {
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("이미지 게시판 리스트 데이터 처리 중 오류가 발생되었습니다.");
		} finally {
			// 7. 닫기
	...
	public int update(ImageVO vo) throws Exception {
		...
		try {
			...
			throw new Exception("이미지 게시판 글수정 처리 중 DB오류가 발생되었습니다."); // ImageController에서예외처리를 위한 처리
		} finally {
			// 7. 닫기
			DBInfo.close(con, pstmt, rs);
		} // end of try~catch ~ finally
	...
	public int delete(long no) throws Exception {
		...
		try {
			...
			e.printStackTrace(); // 개발자를 위한 출력
			throw new Exception("이미지 게시판 글삭제 처리 중 DB오류가 발생되었습니다."); // ImageController에서예외처리를 위한 처리
		} finally {
			// 7. 닫기
			DBInfo.close(con, pstmt, rs);
		} // end of try~catch ~ finally
