	...
	public void execute() {
		...
		while (true) {
			try {
				...
				// 메뉴 출력
				Output.menu("1.공지 게시판리스트  2.공지 게시판글보기  3.공지 게시판글쓰기",
						"4.공지 게시판글수정  5.공지 게시판글삭제  0.이전메뉴",
						"글보기, 글수정, 글삭제를 하시려면 '1.공지 게시판리스트'에서 먼저 글번호 확인요망");
				// 메뉴 입력
				...
				switch (menu) {
					case "1":
						...
						// 1. DB에서 데이터 가져오기
						List<NoticeVO> list =
								(List<NoticeVO>) ExeService.execute(new NoticeListService(), null);
					...
					case "3":
						...
						int writeResult =
								(Integer) ExeService.execute(new NoticeWriteService(), invo);
						// 3. 결과에 대한 출력
						NoticePrint.print(writeResult, "공지 게시판 글쓰기");
						break;
					case "4":
						...
						NoticeVO updateVO = (NoticeVO) ExeService.execute(new NoticeViewService(),
								Input.getLong("수정할 글번호 입력"));
						// 3. 화면에 뿌린다. - 글보기 출력 -> 수정처리 (updateNoticeVO)) while 안에서 출력 (4번과 합침)

						// 4. 수정처리를 한다. -> 메서드 작성
						updateNoticeVO(updateVO);
						// 위에 메서드 안에서 vo객체를 변경을 하면 밖에서도 변경이 된다.
						System.out.println("NoticeController.execute() 수정완료 후 vo : " + updateVO);

						// 5. 수정한 내용을 DB에 저장 -> NoticeUpdateService
						int updateResult =
								(Integer) ExeService.execute(new NoticeUpdateService(), updateVO);
						// 6. 수정결과를 출력한다. print(result, msg)
						NoticePrint.print(updateResult, "공지 게시판 글수정");
						break;
					case "5":
						// 삭제할 글번호를 받는다.
						// DB에 삭제처리 - NoticeDeleteService
						int deleteResult = (Integer) ExeService.execute(new NoticeDeleteService(),
								Input.getLong("삭제할 글번호 입력"));
						// 3. 삭제된 결과를 출력한다. print(result, msg)
						NoticePrint.print(deleteResult, "공지 게시판 글삭제");
						break;
					case "0":
						// 이전메뉴
						return;
		...
		while (true) {
			...
			switch (menu) {
				...
				case "0":
					return;
				case "-1":
					throw new Exception("공지 게시판 수정이 취소되었습니다.");
				default:
