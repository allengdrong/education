...
/**
 * <h3>[Out class - JAVA Contsol의 출력에 관련된 객체]</h3>
 * 
 * <p>
 * <h4>line()</h4> 특별한 문자의 반복으로 한줄을 출력하는 메서드<br/>
 * - line(반복문자열, 반복회수)
 * 
 * <h4>greeting()</h4> 프로그램이 시작되거나 끝날때 사용자에게 인사를 해주는 메서드. 위아래 * 라인이 포함<br/>
 * - greeting(인사말 문자열)
 * 
 * <h4>position()</h4> 프로그램의 실행 위치를 출력하는 메서드. 위아래 '-'라인이 포함<br/>
 * - position(위치정보-main>board)
 * 
 * <h4>menu()</h4> 메뉴를 출력한다. 여러줄로 출력하고 싶으면 여러개의 문자열을 전달한다. 앞뒤로 '-'라인이 포함<br/>
 * - menu(메뉴문자열, ... 메뉴문자열)
 * 
 * <h4>exePos()</h4> 실행하는 클래스와 메서드 정보를 출력하는 메서드. 앞뒤로 '***+++ 문자열 +++***' 둘러쌓여있다.<br/>
 * - exePos(실행정보의 문자열)
 * 
 * <h4>title()</h4> 제목을 출력하는 메서드. 위아래 '*' 라인 포함.<br/>
 * - title(제목문자열)
 * 
 * <h4>printWithLine()</h4> 위아래 전달한 문자열의 라인을 포함 시켜서 입력한 문자열을 출력해 주는 메서드.<br/>
