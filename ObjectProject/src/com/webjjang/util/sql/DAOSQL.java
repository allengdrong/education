...
public class DAOSQL {
	...
	// 3) 붙여진 순서 번호 중에서 페이지(1)에 알맞은 데이터 가져오기
			= " select rnum, no, title, id, to_char(writeDate, 'yyyy.mm.dd') writeDate, fileName"
					...
					// 1) 원본 데이터 가져오기
					+ " select no, title, id, writeDate, fileName from image" + " order by no desc "
					+ " ) " + " ) where rnum between 1 and 10";
	...
	// 5. 이미지 게시판 글삭제
	public static final String IMAGE_DELETE = " delete from image where no = ? ";
	...
	// 3) 붙여진 순서 번호 중에서 페이지(1)에 알맞은 데이터 가져오기
			=
			" select rnum, no, title, to_char(startDate, 'yyyy.mm.dd') startDate, to_char(endDate, 'yyyy.mm.dd') endDate, to_char(updateDate, 'yyyy.mm.dd') updateDate"
					+ " from ( "
					// 2) 가져온 원본 데이터에 순서 번호 붙이기
